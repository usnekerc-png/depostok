...
const tracked=(p,l)=>Boolean(inv(p,l)?.is_tracked);
const threshold=(p,l)=>{const r=inv(p,l);return +(r?.min_stock_threshold ?? st.products.find(x=>x.id===p)?.min_stock ?? 0)};
const days=d=>d?Math.ceil((new Date(d+'T23:59:59')-Date.now())/86400000):99999;
const mgr=()=>['admin','manager'].includes(st.profile?.role);
...
function home(){
  const expAll=st.products.filter(p=>days(p.expiry_date)>=0&&days(p.expiry_date)<=14&&st.locations.some(l=>trackedAt(p,l))).length;
  const expired=st.products.filter(p=>days(p.expiry_date)<0&&st.locations.some(l=>trackedAt(p,l))).length;
  const cr=st.products.filter(crit).length;
  const today=st.movements.filter(m=>new Date(m.created_at).toDateString()===new Date().toDateString()).length;
  const bs=st.locations.map(l=>{
    const tp=st.products.filter(p=>trackedAt(p,l)).length;
    const cp=st.products.filter(p=>criticalAt(p,l)).length;
    const ep=st.products.filter(p=>trackedAt(p,l)&&days(p.expiry_date)>=0&&days(p.expiry_date)<=14).length;
    return `<section class="branch clickable" data-loc="${l.id}"><h3><span>${esc(l.name)}</span>${l.is_main?'<span class="pill">Ana Depo</span>':''}</h3><div class="metrics"><div class="metric"><div class="mlabel">Toplam Ürün</div><div class="mvalue">${tp}<span class="munit">ürün</span></div></div><div class="metric"><div class="mlabel">Kritik Stok</div><div class="mvalue">${cp}<span class="munit">ürün</span></div></div><div class="metric"><div class="mlabel">SKT Yaklaşan</div><div class="mvalue">${ep}<span class="munit">ürün</span></div></div></div></section>`
  }).join('');
  shell(`<div class="branch-grid">${bs}</div><div class="section">Hızlı İşlemler</div><div class="actions"><button class="action" data-a="entry"><i>↓</i>Stok Girişi</button><button class="action" data-a="exit"><i>↑</i>Stok Çıkışı</button><button class="action" data-a="transfer"><i>⇄</i>Transfer</button><button class="action" data-a="count"><i>✓</i>Sayım</button></div><div class="stats"><div class="stat click" data-s="critical"><div class="stat-label">Kritik Stok</div><div class="stat-value red">${cr}</div><div class="stat-unit">ürün</div></div><div class="stat click" data-s="expiry"><div class="stat-label">SKT Yaklaşan</div><div class="stat-value gold">${expAll}</div><div class="stat-unit">ürün${expired?` · ${expired} geçmiş`:''}</div></div><div class="stat click" data-s="today"><div class="stat-label">Bugünkü Hareketler</div><div class="stat-value">${today}</div><div class="stat-unit">işlem</div></div><div class="stat click" data-s="all"><div class="stat-label">Toplam Ürün</div><div class="stat-value">${st.products.length}</div><div class="stat-unit">sistemde kayıtlı</div></div></div>`);
  document.querySelectorAll('[data-a]').forEach(x=>x.onclick=()=>move(x.dataset.a));
  document.querySelectorAll('[data-loc]').forEach(x=>x.onclick=()=>{st.page='products';st.filter='loc:'+x.dataset.loc;render()});
  document.querySelectorAll('[data-s]').forEach(x=>x.onclick=()=>{const s=x.dataset.s;if(s==='today')st.page='movements';else{st.page='products';st.filter=s==='critical'?'critical':s==='expiry'?'expiry':'all'}render()});
}
...
function profile(){shell(`<div class="page-title">Profil</div><div class="card">${logo()}<h3>${esc(st.profile?.full_name||'Kullanıcı')}</h3><div class="tiny">${esc(st.user?.email||'')}</div><p><span class="badge warn">${esc(st.profile?.role||'staff')}</span> ${st.profile?.is_approved?'<span class="badge good">Onaylı</span>':'<span class="badge bad">Bekliyor</span>'}</p><div class="tiny">Designed by <b class="gold">anvecan.com</b></div></div>`)}
function render(){if(!st.user)return loginView();if(!st.profile)return load();({home,products,movements,profile}[st.page]||home)()}
